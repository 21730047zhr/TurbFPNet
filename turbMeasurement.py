...
def corr_mat(corr, D, r0, save_path = './data'):
	num_zern = 36
	N_rows = 16
	N_cols = 16

	subC = _nollCovMat(num_zern, D, r0)
...
def tilt_mat(N, D, r0, L, save_path = './data', thre = 0.04, adj = 1, use_temp = False): 
	# N: image size
	# D: Apeture diameter
	# r0: Fried parameter
	...
	if use_temp: 
		...
		np.save(os.path.join(save_path, 'S_half-size_{}-D_r0_{:.4f}_thre_{}.npy'.format(N,D_r0, thre)), data)
